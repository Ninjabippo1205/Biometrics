...
def processDataset(path, gallery_percentage, unkown_percentage, nounknown, forced):
    if not os.path.exists(path):
        print("Dataset could not be loaded, please check folder location")
        exit(-1)

    if os.path.exists('gallery.json') and os.path.exists('unknown.json') and os.path.exists('probe.json') and not forced:
        file = open('gallery.json', 'r')
...
def main():
    ...
    parser.add_argument('-f', '--force', dest="forced", action='store_true', default=False, help="Use to force the creation of a new gallery, probe and unknown. It's most useful when using a different dataset")
    ...
    gallery = processDataset(arguments.dataset_path, arguments.gallery_percentage, arguments.unknown_percentage, arguments.no_unknown, arguments.forced)
